...
## Чем опознаётся участник

В профиле четыре разных «почты и логина», их легко перепутать:

| Поле | Что это | Кто заполняет |
|------|---------|---------------|
| `email` | адрес из `auth.users`: настоящая почта Яндекс ID либо технический `<login>@id.quantumschool.ru` | триггер входа |
| `login` | придуманный логин для входа по паролю; у Яндекс ID — `NULL` | выводится из `email` |
| `yandex_login` | логин Яндекс ID (`ivanov.ii`); им же участник подписан в мониторе Яндекс.Контеста | `sync_oauth_user_profile` при входе |
| `recovery_email` | необязательная почта для восстановления доступа | сам участник при регистрации |
| `contact_email` | почта, указанная внутри анкеты — куда реально писать | инструмент сопоставления |

`profileContactEmail` в `src/lib/profileUtils.ts` выбирает адрес для связи:
`contact_email` → почта аккаунта → `recovery_email`.

## Привязка Яндекс ID к аккаунту с логином

Кто завёл вход по логину, а потом Яндекс ID, раньше получал два разных
кабинета. В профиле есть раздел «Способы входа»: `linkIdentity` добавляет
вторую identity тому же пользователю, профиль и оценки остаются одни.

Чтобы это работало, в Supabase Dashboard нужно:

1. **Authentication → Advanced (Manual Linking)** — включить. Без этого
   `linkIdentity` отвечает ошибкой, а интерфейс покажет «привязка не включена».
2. **Authentication → URL Configuration → Redirect URLs** — добавить адрес
   профиля (`https://<домен>/profile`). Иначе Яндекс вернёт человека на Site URL,
   и он не увидит результат привязки.

Чего привязка **не** делает: слить два уже существующих аккаунта нельзя. Если
Яндекс ID уже заведён на сайте отдельно, Supabase откажет — за этими двумя
кабинетами стоят разные оценки, сдачи и группы, и выбирать за человека, что
из этого оставить, система не должна.

`public.user_yandex_login(uuid)` берёт логин Яндекса из `auth.identities`, а не
из `raw_user_meta_data`: у привязанного аккаунта в метаданных лежит ещё и наш
собственный login. Этой же функцией пользуется защитный триггер — если они
разъедутся, вход у привязанных аккаунтов начнёт падать на проверке.

## Сопоставление ответов форм

Яндекс.Формы не сохраняли автора ответа: подстановка данных авторизованного
пользователя была выключена, колонки «Логин» и «Эмейл» в выгрузках пусты.
Отсюда две вещи в схеме:

- `selection_form_links` — подтверждённые связи «ответ формы ↔ аккаунт».
  Пишет их только суперадмин, через `superadmin_apply_form_links(jsonb)`
  (сотня строк за один вызов, она же переносит почту в `contact_email`).
  Снять связь — `superadmin_clear_form_link(uuid, text)`.
- `selection_stage_config.questionnaire_prefill_param` и `essay_prefill_param` —
  имя параметра адреса, которым сайт подставляет в форму код участника
  (`?answer_short_text_123456=<id аккаунта>`). Пустая строка — выключено.
  Заполните его на новых формах, и разбирать выгрузки больше не придётся.

Разбор файлов идёт в браузере (`src/lib/tableImport.ts`, xlsx читается через
`DecompressionStream`), выгрузки на сервер не уходят. Проверить логику без
браузера: `node scripts/test-matching.mjs`; посмотреть, что вытащит парсер из
реальных файлов в `data.local/` — `node scripts/probe-matching.mjs`.
