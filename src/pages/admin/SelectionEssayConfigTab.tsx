...
export default function SelectionEssayConfigTab() {
  ...
  const handleSaveLink = async () => {
    ...
    const { error: saveError } = await saveSelectionConfig({ essay_form_id: formId }, user.id);
  ...
  const handlePublish = async () => {
    ...
    setSavedFormId(formId);
    setPublished(true);
    setSuccess('Форма опубликована — ученики видят её на этапе 1');
    setSaving(false);
    await load();
  };
  ...
  const handleUnpublish = async () => {
    ...
    const { error: saveError } = await saveSelectionConfig({ essay_published: false }, user.id);
    ...
    setPublished(false);
    setSuccess('Форма снята с публикации — ученики видят заглушку');
    setSaving(false);
    await load();
  };
    ...
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm">
          «Сохранить» — черновик и предпросмотр. «Опубликовать» — форма появится у учеников.
        </p>
      </div>
      ...
      <div className="bg-slate-900/60 border border-white/5 rounded-2xl p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border ${
            ...
            {published ? <CheckCircle className="w-3.5 h-3.5" /> : <FileText className="w-3.5 h-3.5" />}
