 ...
 *
 * «Ответ или отметка» — для рассылок: человек, который отправил форму, но
 * чей ответ мы не смогли сопоставить, писем-напоминаний получать не должен.
 * «Только найденный ответ» — для сверки: показывает, где у нас реально лежит
 * работа, а где мы верим на слово.
 */
export type StageBasis = 'answer_or_mark' | 'answer';

export const STAGE_BASIS_OPTIONS: { value: StageBasis; label: string }[] = [
  { value: 'answer_or_mark', label: 'Ответ или отметка на сайте' },
  { value: 'answer', label: 'Только найденный ответ' },
];

export const STAGE_BASIS_HINTS: Record<StageBasis, string> = {
  answer_or_mark: 'Выполнено = ответ сопоставлен или человек отметил отправку на сайте.',
  answer: 'Выполнено = ответ формы сопоставлен с аккаунтом. Отметки на сайте не в счёт.',
};
